...
ConnectionList.prototype.addConnection = function(connection, key) {
  ...
  connection.on("message", (message) => {
    if (message.type === "utf8") {
      ...
      this.emit("message", data, data.ID);
    }
  });
...
ConnectionList.prototype.send = function(key, id, data) {
  if (typeof this.connections[key] !== "undefined") {
    this.connections[key].rawConnection.sendUTF(JSON.stringify({
      ID: id,
      content: data
    }));
  }
...
ConnectionList.prototype.sendAll = function(id, data) {
  Object.keys(this.connections).forEach(key => {
    this.send(key, id, data);
  });
};
